...
function second1() {
  ...
  for (e of input2) {
    ...
    for (let i = 0; i < e.length; i++) {
      const found = letterOccurances.find(letter => letter.name === e[i]);
      if (found) {
        found.value++;
      } else {
        letterOccurances.push({ name: e[i], value: 1 });
      }
    }
    if (letterOccurances.find(letter => letter.value === 2)) {
      two++;
    }

    if (letterOccurances.find(letter => letter.value === 3)) {
      three++;
    }
  }
  return two * three;
  ...
  for (first of input2) {
    for (second of input2) {
      ...
      if (changes === 1) {
        for (let i = 0; i < first.length; i++) {
          if (first[i] !== second[i]) {
            return first.substring(0, i) + first.substring(i + 1, first.length);
          }
        }
      }
    }
